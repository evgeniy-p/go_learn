...
func TestGame0(t *testing.T) {
	for caseNum, commands := range game0cases {
		logic.InitGame("reset game") // сбрасываем состояние игры
		data, rooms, err := logic.InitGame("reset game")
		if err != nil {
			t.FailNow()
		}
		for _, item := range commands {
			fmt.Println(item.command)
			answer, err := logic.HandleCommand(item.command, data, rooms) // внутри handleCommand отрабатывает ваш код
			if err != nil {
				t.FailNow()
			}
